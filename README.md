...
<p align="center">
  This repository reproduces the Isaac Lab G1 locomanipulation imitation learning pipeline and explores three low-dimensional imitation policies on the same G1 Mimic-generated demonstrations.<br>
  本项目复现 Isaac Lab G1 人形机器人移动操作模仿学习流程，并基于同一批 G1 Mimic demonstration 对比 BC-RNN、Diffusion Policy 与 ACT 三类低维模仿学习策略。
</p>
...
The project currently contains three parts:
...
This repository focuses on **G1 locomanipulation imitation learning reproduction and policy comparison**, rather than proposing a new algorithm.
...
The current results should be interpreted as reproduction and engineering validation. The evaluation is still limited to simulation and a relatively fixed task setting.
...
- [ ] Add more systematic failure-mode statistics.
...
本仓库记录我对 **NVIDIA Isaac Lab 中 G1 pick-place / locomanipulation 任务** 的复现、调试与策略对比过程。
...
本仓库主要关注 **G1 移动操作模仿学习流程复现与策略对比**，不声称提出新的算法。
...
当前结果更适合理解为复现与工程验证。评估仍然局限在仿真和相对固定的任务设置下。
...
- [x] 使用 1000 条 Mimic demonstration 训练并评估 robomimic BC-RNN。
- [x] 将 G1 demonstration 转换为 low-dimensional zarr 数据集。
- [x] 训练并部署 low-dimensional Diffusion Policy，完成 IsaacLab 闭环 rollout。
- [x] 训练并部署 low-dimensional ACT，完成 IsaacLab 闭环 rollout。
